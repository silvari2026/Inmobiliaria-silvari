...
function guardarFavoritos() { localStorage.setItem('silvari_favoritos', JSON.stringify(favoritos)); }
...
function abrirWhatsApp() {
    window.open('https://wa.me/5355415537?text=Hola%20me%20interesa%20conocer%20mas%20sobre%20los%20inmuebles', '_blank');
}

// ===== ETIQUETA =====
function obtenerEtiqueta(propiedad) {
    if (propiedad.estado === 'vendido') return { clase: 'vendido', texto: 'Vendido' };
    if (propiedad.categoria === 'local') return { clase: 'local', texto: 'Local Comercial' };
    if (propiedad.categoria === 'garaje') return { clase: 'garaje', texto: 'Garaje' };
    if (propiedad.categoria === 'terreno') return { clase: 'terreno', texto: 'Terreno' };
    if (propiedad.categoria === 'habitacion') return { clase: 'habitacion', texto: 'Habitacion' };
...
function crearTarjeta(propiedad) {
    ...
    articulo.innerHTML = '<div class="tarjeta-imagen-container">' +
        ...
        (propiedad.habitaciones > 0 ? '<span class="caracteristica"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg> ' + propiedad.habitaciones + ' Hab.</span>' : '') +
        (propiedad.banos > 0 ? '<span class="caracteristica"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg> ' + propiedad.banos + ' Banos</span>' : '') +
...
// ===== DETALLE =====
var imagenActual = 0, propiedadDetalle = null;
...
function cargarDetalle() {
    ...
    if (!propiedadDetalle) {
        contenedor.innerHTML = '<div class="container" style="text-align:center;padding:80px 0"><h2>Inmueble no encontrado</h2><p style="margin:16px 0">El inmueble que buscas no esta disponible.</p><a href="index.html" class="btn-cta">Volver al Inicio</a></div>';
        return;
    }
    ...
    contenedor.innerHTML = '<div class="container">' +
        '<div class="breadcrumb"><a href="index.html">Inicio</a><span> › </span><a href="index.html#propiedades">Inmuebles</a><span> › </span><span class="actual">' + propiedadDetalle.titulo + '</span></div>' +
        ...
        '<button class="galeria-btn" onclick="cambiarImagen(1)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg></button></div></div>' +
        '<div class="detalle-grid"><div class="detalle-info">' +
        ...
        '<span class="detalle-etiqueta ' + etiqueta.clase + '">' + etiqueta.texto + '</span>' +
        '<div class="detalle-precio">' + propiedadDetalle.precioMostrar + '</div>' +
        ...
        (propiedadDetalle.habitaciones > 0 ? '<div class="detalle-caracteristica"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg><div class="detalle-caracteristica-texto"><strong>' + propiedadDetalle.habitaciones + '</strong>Habitaciones</div></div>' : '') +
        (propiedadDetalle.banos > 0 ? '<div class="detalle-caracteristica"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg><div class="detalle-caracteristica-texto"><strong>' + propiedadDetalle.banos + '</strong>Baños</div></div>' : '') +
        ...
        '</div><div class="detalle-sidebar"><div class="sidebar-card">' +
        '<h3>¿Te interesa este inmueble?</h3>' +
        '<div class="sidebar-precio">' + propiedadDetalle.precioMostrar + '</div>' +
        '<a href="https://wa.me/5355415537?text=' + encodeURIComponent('Hola, me interesa el inmueble: ' + propiedadDetalle.titulo + ' (' + propiedadDetalle.precioMostrar + ')') + '" class="btn-contactar btn-whatsapp-detalle" target="_blank"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/></svg> Contactar por WhatsApp</a>' +
        ...
        '<a href="index.html#propiedades" class="btn-volver"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg> Volver a Inmuebles</a>' +
        '</div></div></div></div>';
}
